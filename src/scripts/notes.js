            ...
            (function () {
                ...
                var T = {
                    en: {
                        ...
                        intro: "Open any talk’s “📄 Full notes”, then select a sentence (on a phone, tap a sentence — tap again to extend across more sentences) and choose “★ Save as note”. Your saved notes are grouped by talk below and stored in this browser. Click a note to jump back to it in the original talk.",
                        empty: "No notes yet. Open a talk’s full notes, select a sentence (or tap one on a phone), and choose “★ Save as note” to collect it here.",
                        talkNo: "Talk #",
                        noteOne: " note",
                        noteMany: " notes",
                ...
                Array.prototype.slice
                    ...
                    .forEach(function (lb) {
                        ...
                        talkMeta[m[1]] = {
                            title: h ? h.textContent.trim() : T.talkNo + m[1],
                ...
                function highlightNote(body, note) {
                    ...
                    /* Cross-language / multi-block fallback: the exact text isn't
                       in this body (the note was saved in the other language, or
                       spans multiple paragraphs), so highlight the anchored
                       top-level block(s) by index instead. */
                    if (typeof note.b0 === "number" && note.b0 >= 0) {
                        var kids = body.children;
                        var endB =
                            typeof note.b1 === "number" && note.b1 >= note.b0
                                ? note.b1
                                : note.b0;
                        var firstMark = null;
                        for (
                            var bi = note.b0;
                            bi <= endB && bi < kids.length;
                            bi++
                        ) {
                            var br = document.createRange();
                            br.selectNodeContents(kids[bi]);
                            var bmark = document.createElement("mark");
                            bmark.className = "note-hl";
                            bmark.setAttribute("data-note-id", note.id);
                            try {
                                br.surroundContents(bmark);
                                firstMark = firstMark || bmark;
                            } catch (e) {}
                        }
                        return firstMark;
                    }
                ...
                function paintRange(range, className, noteId) {
                    ...
                    nodes.forEach(function (node) {
                        // Skip whitespace-only nodes (the newlines/indentation
                        // between block elements) so a multi-paragraph selection
                        // doesn't paint thin highlight bars on the empty gaps.
                ...
                var pending = null;
                /* Index of the top-level .lb-body block (<p>/<ol>/<ul>) that
                   contains a node — a language-agnostic anchor, so a note can be
                   re-highlighted at the corresponding paragraph when the reader
                   opens the talk in the other language. */
                function topBlockIndex(node, body) {
                    var el = node.nodeType === 3 ? node.parentNode : node;
                    while (el && el.parentNode !== body) el = el.parentNode;
                    if (!el || el === body) return -1;
                    return Array.prototype.indexOf.call(body.children, el);
                }
                function buildCapture(range) {
                    if (!range || range.collapsed) return null;
                    var body = closestBody(range.commonAncestorContainer);
                    if (!body) return null;
                    var b0 = topBlockIndex(range.startContainer, body);
                    var b1 = topBlockIndex(range.endContainer, body);
                ...
                toolbar.innerHTML =
                    '<button type="button" class="nt-save">' + T.saveAsNote + "</button>";
                ...
                function saveSelection() {
                    ...
                    var note = {
                        ...
                        talk: cap.talk,
                        title: meta.title || T.talkNo + cap.talk,
                        sc: meta.sc || "",
                        text: cap.text,
                        raw: cap.raw,
                        b0: cap.b0,
                ...
                function deleteNote(id) {
                    ...
                    if (body) {
                        var marks = body.querySelectorAll(
                            'mark.note-hl[data-note-id="' + id + '"]',
                        );
                        Array.prototype.forEach.call(marks, unwrap);
                    }
                    renderNotes();
                }
                ...
                section.innerHTML =
                    '<div class="wrap">' +
                    '<div class="sec-head"><h2>' + T.yourNotes + "</h2></div>" +
                ...
                function renderNotes() {
                    ...
                    if (!notes.length) {
                        ...
                        ids.forEach(function (tid) {
                            ...
                            h3.textContent = meta.title || T.talkNo + tid;
                            link.appendChild(h3);
                            ...
                            cnt.textContent =
                                g.length + (g.length === 1 ? T.noteOne : T.noteMany);
